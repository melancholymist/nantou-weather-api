...
app.get("/", (req, res) => {
  res.json({
    message: "歡迎使用南投埔里 3 天天氣健康提醒 API",
    version: "0.2.0",
    description:
      "這是一個提供南投縣埔里鎮未來 3 天天氣預報與長者健康建議的小型 API 服務。",
    source: {
      provider: "中央氣象署開放資料平臺",
      datasetId: "F-D0047-023",
      location: "南投縣埔里鎮"
    },
    endpoints: {
      "/api/health": "服務健康檢查。回傳 { status: 'ok' } 代表服務正常。",
      "/api/weather/puli":
        "埔里鎮未來 3 天整合預報。包含日期、溫度範圍、濕度、12 小時降雨機率與一句健康建議。",
      "/api/debug/cwa-puli":
        "（開發用）回傳中央氣象署原始 JSON 資料結構摘要。",
      "/api/debug/cwa-puli-days":
        "（開發用）回傳整理後的 3 天天氣摘要（不含健康建議）。"
    },
    note:
      "本服務僅作為生活提醒使用，實際天氣與健康問題請以醫療專業與官方預報為主。"
  });
});
...
// 👋 根路由：先用超簡單版本確認有沒有被註冊
app.get("/", (req, res) => {
  res.send("這是根路由 / 測試頁，代表 Express 有成功找到這條路。");
});
